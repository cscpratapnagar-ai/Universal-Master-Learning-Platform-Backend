...
public class AssessmentIntelligenceEngine {
    public AssessmentIntelligenceResponse analyze(UUID enrollmentId, List<AssessmentAttempt> attempts) {
        ...
        if (evidence.isEmpty()) return new AssessmentIntelligenceResponse(enrollmentId, "NOT_READY", "NO_DATA", 0, 0, 0, 0, false,
                "Complete an assessment to establish a readiness baseline", List.of());
        ...
        return new AssessmentIntelligenceResponse(enrollmentId, readiness, trend, round(average), latest, evidence.size(), passed,
                retake, recommendation, insights);
    }
    private AssessmentIntelligenceResponse.AssessmentInsight insight(List<AssessmentAttempt> attempts) {
        AssessmentAttempt latest = attempts.getFirst(); Assessment a = latest.getAssessment(); String trend = trend(attempts);
        ...
        return new AssessmentIntelligenceResponse.AssessmentInsight(a.getId(), a.getTitle(), a.getAssessmentLevel(), attempts.size(),
                latest.getScore(), latest.isPassed(), trend, readiness, recommendation);
    }
    private String trend(List<AssessmentAttempt> attempts) {
        if (attempts.size() < 2) return "STABLE";
        int latest = attempts.get(0).getScore(), previous = attempts.get(1).getScore();
