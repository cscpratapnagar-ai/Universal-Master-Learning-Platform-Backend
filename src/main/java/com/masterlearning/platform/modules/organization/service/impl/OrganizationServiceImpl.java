...
public class OrganizationServiceImpl implements OrganizationService {
 ...
 @Transactional(readOnly=true) public List<OrganizationMemberResponse> getMembers(UUID organizationId){find(organizationId);return members.findAll().stream().filter(m->m.getOrganization().getId().equals(organizationId)).map(m->new OrganizationMemberResponse(m.getId(),m.getUser().getId(),m.getUser().getEmail(),m.getUser().getFirstName(),m.getUser().getLastName(),m.isActive())).toList();}
